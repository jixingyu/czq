...
class App_user_lib
{
    ...
    public function __construct()
    {
        ...
        $this->CI->load->model('app_user_token_model');
    }
    ...
    public function is_logged_in($token = '')
    {
        ...
        if ($token) {
            $result = $this->CI->app_user_token_model->get_one(array(
                'token' => $token
            ));
            if ($result) {
    ...
    public function verify_token($token)
    {
        ...
        if (!$result) {
            ...
            // delete unused token
            $this->CI->app_user_token_model->delete(array(
            ...
            $this->CI->app_user_token_model->update(array(
                'token'     => $token,
                'expires'   => $expires
            ), array(
                'user_id'   => $uid
    ...
    public function login($params = array())
    {
        ...
        if (!empty($user)) {
            if ($user['is_active'] == 0) {
                $this->CI->load->model('myconfig_model');
                $config = $this->CI->myconfig_model->get_one(array());
            ...
            $token   = $this->_gen_token($uid);
            $expires = strtotime('1 month');
            $this->CI->app_user_token_model->replace(array(
                'user_id' => $uid,
                'token'   => $token,
                'created' => time(),
                'expires' => $expires
